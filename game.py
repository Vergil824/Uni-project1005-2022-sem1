...
pygame.init()
# created a sound variable called point_sound
point_sound = pygame.mixer.Sound('./sound/point.wav')


def point():  # initialised a function to play point_sound
...
class Snake:
    ...
    def initialize(self):
        ...
        # resets the position of the snake after death to prevent snake from crashing into itself
        self.facing = 'right'
        self.score = 0

    # highscore feature coded by wshi9637
    def high_score(self):
        f = open('scorelog.txt', 'r')
        high_score = f.read()
...
class Game:
    ...
    def do_move(self, move):
        ...
        if self.snake.position == self.strawberry.position:
            ...
            reward = 1
            self.snake.score += 1
            point()  # Plays the sound effect if the snake score increases

            # updates high score file if current score is higher
            if self.snake.score > int(self.snake.high_score()):
                high_score_sound = pygame.mixer.Sound('./sound/highscore.wav')
                high_score_sound.set_volume(0.4)
                pygame.mixer.Sound.play(high_score_sound)

                f = open('scorelog.txt', 'w')
                print(self.snake.score, file=f, end='')
                f.close()
    ...
    def blit_score(self, color, screen):
        font = pygame.font.Font('./font/Regular.otf', 20)
        text = font.render('Score: ' + str(self.snake.score), True, color)
        screen.blit(text, (0, 0))

    # displays high score and updates it live.
    def blit_high_score(self, color, screen):
        f = open('scorelog.txt', 'r')
        high_score = f.read()
        f.close()
        font = pygame.font.Font('./font/Regular.otf', 20)
        text = font.render('Highscore: ' + str(high_score), True, color)
